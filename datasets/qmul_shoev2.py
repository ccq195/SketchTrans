...
class SHOE(BaseImageDataset):
    def __init__(self,root,trial):
        ...
        self.gallery = []
        self.query = []
        train_visible_path = root+'ShoeV2/train_visible.txt'
        train_sketch_path = root+'ShoeV2/train_sketch.txt'
        test_visible_path = root+'ShoeV2/test_visible.txt'
        test_sketch_path = root+'ShoeV2/test_sketch.txt'
        ...
        for j in range(len(data_file_list)):
            ...
            cam = int(0)
            self.train2.append((img, pid2label_img[id], file_cam[cam],0))
        ...
        for j in range(len(data_file_list)):
            ...
            id = int(data_file_list[j].split(' ')[1])
            cam = int(1)
            self.train2.append((img, pid2label_img[id], cam,1))
            self.train[pid2label_img[id]].append((img, pid2label_img[id], cam,1))

        self.num_train_pids, self.num_train_imgs, self.num_train_cams, self.num_train_vids = self.get_imagedata_info(self.train2)
        ...
        for j in range(len(data_file_list)):
            ...
            id = int(data_file_list[j].split(' ')[1])
            cam = int(0)
            self.gallery.append((img, id, cam,0))
