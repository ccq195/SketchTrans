...
class TuberlinDataset(Dataset):
    def __init__(self, mode='train', transform=None,  stransform=None):
        ...
        if mode == 'train':
            file_ls_p = '/data0/data_ccq/zero-shot_SBIR_dataset/TUBerlin/zeroshot/ImageResized_ready_filelist_train.txt'
            file_ls_s = '/data0/data_ccq/zero-shot_SBIR_dataset/TUBerlin/zeroshot/png_ready_filelist_train.txt'
            file_ls_s2 = '/data0/data_ccq/zero-shot_SBIR_dataset/TUBerlin/zeroshot/png_ready_filelist_test.txt'
            ...
            img_paths = [os.path.join('/data0/data_ccq/zero-shot_SBIR_dataset/TUBerlin', f) for f in img_paths]
            ...
            if mode == "gallery":
                file_ls = '/data0/data_ccq/zero-shot_SBIR_dataset/TUBerlin/zeroshot/ImageResized_ready_filelist_zero.txt'
                ...
                file_ls = '/data0/data_ccq/zero-shot_SBIR_dataset/TUBerlin/zeroshot/png_ready_filelist_zero.txt'
                with open(file_ls, 'r') as fh:
                    file_content = fh.readlines()
                img_paths= np.array([' '.join(ff.strip().split()[:-1]) for ff in file_content])
                img_paths = [os.path.join('/data0/data_ccq/zero-shot_SBIR_dataset/TUBerlin', f) for f in img_paths]
                ...
                num_ids = len(selected_ids)
                print('sketchy gallery: {}'.format(len(img_paths)))
    ...
    def __getitem__(self, item):
        ...
        if self.mode == 'train':
            ...
            return img2, imgg, torch.from_numpy(np.array(label)), torch.from_numpy(np.array(cam))
