...
import model.backbones.metro.modeling.data.config as cfg
...
class METRO_Body_Network(torch.nn.Module):
    ...
    def forward(self, images, smpl, mesh_sampler, meta_masks=None, is_train=False):
        ...
        image_feat = self.backbone(images)#1*2048*7*7
        ...
        img_tokens = self.conv_learn_tokens(image_feat_newview)#1*49*2048 -->  1*445*2048

        # concatinate image feat and template mesh
        features = torch.cat([ref_vertices, img_tokens], dim=2)#1*445*3 1*445*2048
        ...
        # forward pass
        if self.config.output_attentions==True:
            features, hidden_states, att = self.trans_encoder(features)#1*445*3
        else:
            features = self.trans_encoder(features)

        pred_3d_joints = features[:,:num_joints,:]#1*14*3
        pred_vertices_sub2 = features[:,num_joints:,:]#1*431*3

        # learn camera parameters
        x = self.cam_param_fc(pred_vertices_sub2)
        x = x.transpose(1,2)
        x = self.cam_param_fc2(x)
        x = self.cam_param_fc3(x)#1*1*3
        cam_param = x.transpose(1,2)
        cam_param = cam_param.squeeze()#3
        ...
        pred_vertices_full = pred_vertices_full.transpose(1,2)#1*6890*3
